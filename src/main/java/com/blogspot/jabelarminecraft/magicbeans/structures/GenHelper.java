...
public class GenHelper
{
	...
	public static final boolean addItemToTileInventory(World world, ItemStack itemstack, int x, int y, int z)
	{
		...
		for (int i = 0; i < inventory.getSizeInventory() && remaining > 0; ++i)
		{
			...
			if (slotstack == null && inventory.isItemValidForSlot(i, itemstack))
			{
				...
				inventory.markDirty(); // was onInventoryChanged();
			}
	...
	public static final int setHangingEntity(World world, ItemStack hanging, int x, int y, int z)
	{
		...
		FakePlayer player = (FakePlayer)new EntityPlayerMP(FMLCommonHandler.instance().getMinecraftServerInstance(), FMLCommonHandler.instance().getMinecraftServerInstance().worldServerForDimension(world.provider.dimensionId), new GameProfile(UUID.randomUUID(), "fake"), new ItemInWorldManager(world));//FakePlayer(world,"fake");
...
//			if (rot > -1) { skull.setSkullRotation(rot % 16); }
	 ...
	 */
	public static final int getMetadata(int rotations, int id, int origMeta)
	{
		if (BlockRotationData.getBlockRotationType(id) == null) return 0;
		...
		for (int i = 0; i < rotations; ++i)
		{
			...
			switch(BlockRotationData.getBlockRotationType(id)) {
			case ANVIL:
				meta ^= 1;
				break;
			case DOOR:
	...
	public static final void setMetadata(World world, int x, int y, int z, int origMeta)
	{
		...
		if (BlockRotationData.getBlockRotationType(id) == null) { return; }
	 ...
	 */
	public static final boolean materialsMatch(int realID, int worldID) {
		return  (Block.getBlockById(worldID).getMaterial() == Material.grass && Block.getBlockById(Math.abs(realID)).getMaterial() == Material.ground) ||
				(Block.getBlockById(worldID).getMaterial().isLiquid() && Block.getBlockById(Math.abs(realID)).getMaterial() == Block.getBlockById(worldID).getMaterial()) ||
				(Block.getBlockById(worldID).getMaterial() == Material.ice && Block.getBlockById(Math.abs(realID)).getMaterial() == Material.water) ||
				(Block.getBlockById(worldID).getMaterial() == Material.piston && Block.getBlockById(Math.abs(realID)).getMaterial() == Material.piston) ||
				(Block.getBlockById(worldID) instanceof BlockRedstoneTorch && Block.getBlockById(Math.abs(realID)) instanceof BlockRedstoneTorch) ||
				(Block.getBlockById(worldID) instanceof BlockRedstoneRepeater && Block.getBlockById(Math.abs(realID)) instanceof BlockRedstoneRepeater);
	}
}